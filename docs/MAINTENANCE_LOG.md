...
2. **回應延遲過長**（`src/graph.py` 的 `agent` 節點）
   改為 LLM 自主決策後，每題的 LLM 呼叫次數從 3 次增為至少 4 次（多一次 agent 決策），LLM 決定補抓時再多 1-2 輪。實測本機 `qwen3.5:9b` 單題總耗時 186 至 470 秒（視是否觸發補抓與機器負載而異），節點分佈以 186 秒的那次為例：`extract_filters` 5 秒、`agent` 決策兩輪合計 93 秒、`generate` 87 秒、實際檢索不到 1 秒——瓶頸全在本地模型推理，非架構本身。可行方向：換用推理更快的模型或量化版本、合併 `rewrite_question`/`extract_filters` 為單次呼叫、資料明顯足夠時跳過 agent 迴圈（但這等於把部分決策權收回程式，與 09-07 改造的目標相衝突，需權衡）。
...
## 2026-09-08　顯示模型推理過程的可行性評估

### 背景

在回答中顯示模型的推導過程，可提升財報問答的可信度，即時串流也能在本機模型的長等待中提供進度感。評估後不納入，記錄依據以免日後重複同一輪評估。

### 排查過程

初始假設「推理段以 `<think>` 標籤內嵌在回覆中，需自行剝除」是錯的，且讓第一版實作失效——據此寫的標籤解析器單元測試全過，卻永遠收不到資料。實際上推理段走 `additional_kwargs["reasoning_content"]` 獨立欄位：未指定 `reasoning` 時模型照樣推理但內容被丟棄，設為 `True` 才會回傳。確認欄位後改為直接讀取，折疊區塊也改用 Chainlit `cl.Step` 原生的 `default_open` / `auto_collapse`，不自行拼裝 HTML。

### 量測結果

功能可行，但延遲代價無法接受。單題端到端（本機 `qwen3.5:9b`，真實 MCP + pgvector）：
...
`extract_filters` 只做代號與文件類型抽取，開推理卻慢了 80 倍，全節點開啟因此最差。該組答案為 0 字則是另一個問題：Ollama 預設 `num_ctx` 為 4096，RAG prompt 加上推理即塞滿，答案被擠成空字串。加大到 16384 可解，但模型不再受限後為一句營收提問寫了 30891 字推理（答案的 21 倍），`generate` 隨之慢了 12 倍。體感延遲也未改善——關閉推理 108.7 秒就看得到答案，開啟後首個推理字元要等 237.6 秒。
...
維持 `reasoning=False`。此限制源於 `qwen3.5:9b` 的推理成本而非架構，換模型後值得重新評估。
...
5 個測試檔與雙語字串鍵值對照全數通過；容器重建後確認行為與基準一致。
