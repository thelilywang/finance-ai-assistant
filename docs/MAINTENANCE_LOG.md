...
初期為求快速打通 tool-calling 與引用組裝，`search_knowledge_base` 的摘要與結構化 metadata 統一封裝、全量送進 context。實測 metadata 佔回傳內容的 57%（7177 字中的 4064 字），且 agent 每迭代一輪就整份重送——而 tool 說明早已寫明叫模型只讀摘要。改法是權責分離：送進 LLM 的複本只留 `summary_for_llm`，完整 metadata 留在 state 供 `assemble` 組裝引用。

**未帶來可量測的延遲改善**，須寫明以免誤讀：瓶頸是本地 9B 模型的生成速度（穩定在每秒約 10 字），減少輸入 token 不影響輸出速度。效益是多輪對話的 context 膨脹速率降低一半以上，降低撐爆 context window 的風險——09-08 已記錄過 `num_ctx` 塞滿時答案會被擠成空字串。

### 二、資料明顯足夠時跳過 agent 第二輪（`route_after_tools`）

第二輪 agent 的唯一產出是模型說一句「夠了，停」，實測要價 82 至 106 秒。`tools -> agent` 改為條件路由，判準與 `src/mcp_server.py` 的 tool docstring 一致，避免程式與 tool 說明各執一套。

與 09-07「決策權交給 LLM」的衝突刻意控制在最小範圍：**只收回「資料明顯足夠 → 停止呼叫工具」**，查無資料、沒有新聞、新聞過期、日期不明、剛補抓完一律交還 agent，補抓決策權未動。誤判代價不對稱（誤判收工會拿過期資料作答，誤判回 agent 只多花一輪），故判準取保守側。

實測（`qwen3.5:9b`，真實 MCP + pgvector，「AAPL 的營收表現如何？」）：改動前 `agent` 第二輪 81.7 秒、總耗時 208.5 秒；改動後該輪未執行、總耗時 204.7 秒。省下一次 LLM 呼叫是結構性的、每次成立，但總耗時只差 3.8 秒——同批量測中 `generate` 在 88 至 163 秒間浮動，機器負載的變異蓋過了省下的時間。

### 三、決策卡字數約束——實測後不採用

原假設：不犧牲 14 個欄位，改用硬性字數約束（每欄限 1-2 句）壓縮生成字數。同容器、同檢索結果、`temperature=0` 實測結果與假設相反：
...
| 項目 | 改動說明 | commit |
|---|---|---|
| Context 權責分離 | `src/graph.py` 新增 `_trim_for_llm()`；裁切失敗時原樣送出並印訊息，不中斷對話。 | `a8a7a69` |
